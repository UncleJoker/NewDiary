...
- (NSArray *)arrayNumButton {
    if (!_arrayNumButton) {
        ...
        for (int i = 1; i <= 9; i++) {
            DWPasswordNumberButton *button = [[DWPasswordNumberButton alloc] initWithFrame:CGRectMake(40 + (25 + DWPasswordNumButtonWidth) * ((i - 1) % 3), DWPasswordNumButtonTopPadding + (DWPasswordNumButtonMargin + DWPasswordNumButtonWidth) * ((i - 1) / 3), DWPasswordNumButtonWidth, DWPasswordNumButtonWidth) number:i];
            button.tag = i;
            [button addTarget:self action:@selector(buttonNumberTap:) forControlEvents:UIControlEventTouchUpInside];
            [array addObject:button];
        }
        DWPasswordNumberButton *button = [[DWPasswordNumberButton alloc] initWithFrame:CGRectMake(DWScreenWidth/2-DWPasswordNumButtonWidth/2, DWPasswordNumButtonTopPadding + (DWPasswordNumButtonMargin + DWPasswordNumButtonWidth) * 3, DWPasswordNumButtonWidth, DWPasswordNumButtonWidth) number:0];
        button.tag = 0;
